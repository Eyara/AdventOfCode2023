...
def get_hand_dict(hand: str) -> dict:
    ...
    updated_d = {k: v for k, v in sorted(d.items(), key=lambda x: x[1], reverse=True)}
    sorted_keys = list(updated_d.keys())

    if len(sorted_keys) == 0:
        updated_d['A'] = 5
    else:
        updated_d[sorted_keys[0]] += hand.count('J')
...
custom_sort_arr = ['J'] + [str(x) for x in range(2, 10)] + ['T', 'Q', 'K', 'A']
